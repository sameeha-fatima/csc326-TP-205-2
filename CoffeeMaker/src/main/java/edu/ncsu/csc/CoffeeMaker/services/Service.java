 ...
 *
 * @param <T> Type of entity that will be handled by this service
 * @param <K> Type of the key for this entity
 *
...
abstract public class Service<T extends DomainObject, K> {
	...
	/**
	 * Returns the Repository that Spring uses for interacting with the database.
	 * This is the only method that _must_ be overridden in child Service classes.
	 *
	...
	/**
	 * Saves the provided object into the database. If the object already exists,
	 * `save()` will perform an in-place update, overwriting the existing record.
	 *
	 * @param obj The object to save into the database.
	 */
	...
	/**
	 * Returns all records of this type that exist in the database. If you want more
	 * precise ways of retrieving an individual record (or collection of records)
	 * see `findBy(Example)`
	 *
	...
	/**
	 * Saves a collection of elements to the database. If an error occurs saving any
	 * of them, no objects will be saved. This makes it handy for ensuring database
	 * consistency where all records should exist together.
	 *
	 * @param objects A List of objects to save to the database.
	 */
	 ...
	 * database. This is faster than, and should be preferred to, `findAll().size()`
	 * if you don't care about the actual records themselves.
	 *
	...
	/**
	 * Provides support for retrieving one or several elements from the database by
	 * passing in an Example describing what you want to see. See
	 ...
	 * 
	 * @param id Id to check for existence of
