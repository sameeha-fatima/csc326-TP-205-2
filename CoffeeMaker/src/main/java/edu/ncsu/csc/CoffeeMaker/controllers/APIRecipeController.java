...
public class APIRecipeController extends APIController {
	 ...
	 * the path variable provided (the name of the recipe desired), with an updated
	 * price and list of ingredients
	 *
	 * @param r represents Recipe object with updated price and ingredients should
	 *          have
	 * @return response to the request
	 */
	@PutMapping(BASE_PATH + "/recipes")
	public ResponseEntity editRecipe(@RequestBody final Recipe r) {
		if (r == null || r.getName() == null) {
			return new ResponseEntity(errorResponse("Recipe name is null"), HttpStatus.BAD_REQUEST);
		}
		final Recipe recipe = service.findByName(r.getName());
		if (null == recipe) {
			return new ResponseEntity(errorResponse("No recipe found with name " + r.getName()), HttpStatus.NOT_FOUND);
		}
		try {
			recipe.editRecipe(r);
			service.save(recipe);
