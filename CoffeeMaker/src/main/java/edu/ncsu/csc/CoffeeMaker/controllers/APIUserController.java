...
public class APIUserController extends APIController {
	...
	public ResponseEntity createUser(@RequestBody final User user) {
		if (null != userService.findByName(user.getName())) {
			return new ResponseEntity(errorResponse("User with the id " + user.getName() + " already exists"),
					HttpStatus.CONFLICT);
		}
//        final User newUser = null;
		try {
			...
			return new ResponseEntity(
					errorResponse("Could not create " + user.getName() + " because of " + e.getMessage()),
	...
	public ResponseEntity login(@RequestBody final User user) {
		final User foundUser = userService.findByName(user.getName());
		if (null == foundUser) {
			new ResponseEntity(errorResponse("No User found for username " + user.getName()), HttpStatus.NOT_FOUND);
		}
